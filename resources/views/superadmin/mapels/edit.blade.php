    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-xl sm:rounded-lg p-6">
                <form method="POST" action="{{ route('superadmin.mapels.update', $mapel->id) }}">
                    @csrf
                    @method('PUT')
                    ...
                    <!-- Mapel ID -->
                    <div class="mb-4">
                        <label for="mapel_id" class="block text-sm font-medium text-gray-600 dark:text-gray-200">
                            ID
                        </label>
                        <input type="text" name="mapel_id" id="mapel_id" class="mt-1 p-2 border rounded-md w-full" required value="{{ $mapel->mapel_id }}" placeholder="Masukkan Nama ID Mata Pelajaran">
                    </div>

                    <!-- Nama Mapel -->
                    <div class="mb-4">
                        <label for="nama_mapel" class="block text-sm font-medium text-gray-600 dark:text-gray-200">
                            Nama Mapel
                        </label>
                        <input type="text" name="nama_mapel" id="nama_mapel" class="mt-1 p-2 border rounded-md w-full" required value="{{ $mapel->nama_mapel }}" placeholder="Masukkan Nama Mapel">
                    </div>

                    <!-- Kategori Mapel -->
                    <div class="mb-4">
                        <label for="kategori" class="block text-sm font-medium text-gray-600 dark:text-gray-200">
                            Kategori
                        </label>
                        <select name="kategori" id="kategori" class="mt-1 p-2 border rounded-md w-full" required>
                    ...
                    <!-- Program Studi -->
                    <div class="mb-4">
                        <label for="prodi" class="block text-sm font-medium text-gray-600 dark:text-gray-200">
                            Program Studi
                        </label>
                        <select name="prodi" id="prodi" class="mt-1 p-2 border rounded-md w-full" required>
                            @foreach ($program_studis as $program_studi)
                                <option value="{{ $program_studi->id }}" {{ $mapel->prodi_id == $program_studi->id ? 'selected' : '' }}>{{ $program_studi->nama_prodi }}</option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="mb-4">
                        ...
                        <div class="flex items-center">
                            <input type="text" id="selected_guru_pengampu" class="border p-2 rounded-md mr-2" readonly placeholder="Pilih Guru Pengampu" value="{{ $mapel->guruPengampu->name }}">
                            <button type="button" class="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded" onclick="openModal()">
                                Pilih Guru Pengampu
                            </button>
                        </div>
                        <input type="hidden" name="guru_pengampu" id="guru_pengampu" required value="{{ $mapel->guru_id }}" />
                    </div>
                    ...
                    <div class="flex items-center justify-between mt-4">
                        ...
                        <!-- Tombol Batal -->
                        <a href="{{ route('superadmin.mapels.index') }}" class="text-gray-600 dark:text-gray-300 hover:underline">Batal</a>
                    </div>
                </form>
            </div>
        </div>
    ...
    <div class="fixed inset-0 z-50 overflow-auto bg-gray-500 bg-opacity-75" id="guruPengampuModal" style="display: none;">
        <div class="relative top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 w-2/4 bg-white rounded shadow-lg">
            ...
            <div class="relative p-6 flex-auto max-h-60 overflow-auto">
                ...
                <!-- Table to display Guru Pengampu Prodi data -->
